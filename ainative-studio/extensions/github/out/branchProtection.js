...
exports.GitHubBranchProtectionProvider = exports.GitHubBranchProtectionProviderManager = void 0;
...
`;
class GitHubBranchProtectionProviderManager {
    set enabled(enabled) {
    ...
    constructor(gitAPI, globalState, logger, telemetryReporter) {
        this.gitAPI = gitAPI;
        this.globalState = globalState;
        this.logger = logger;
        this.telemetryReporter = telemetryReporter;
        this.disposables = new util_1.DisposableStore();
        this.providerDisposables = new util_1.DisposableStore();
        this._enabled = false;
        ...
        this.disposables.add(vscode_1.workspace.onDidChangeConfiguration(e => {
...
exports.GitHubBranchProtectionProviderManager = GitHubBranchProtectionProviderManager;
class GitHubBranchProtectionProvider {
    constructor(repository, globalState, logger, telemetryReporter) {
    ...
    async getRepositoryDetails(owner, repo) {
        const graphql = await (0, auth_1.getOctokitGraphql)();
    ...
    async updateRepositoryBranchProtection() {
        ...
        try {
            for (const remote of this.repository.state.remotes) {
                const repository = (0, util_1.getRepositoryFromUrl)(remote.pushUrl ?? remote.fetchUrl ?? '');
                if (!repository) {
                    continue;
                }
                // Repository details
...
exports.GitHubBranchProtectionProvider = GitHubBranchProtectionProvider;
