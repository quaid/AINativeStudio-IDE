...
class GitHubSourceControlHistoryItemDetailsProvider {
    constructor(_gitAPI, _logger) {
        this._gitAPI = _gitAPI;
        this._logger = _logger;
        this._isUserAuthenticated = true;
        this._store = new Map();
        this._disposables = new util_1.DisposableStore();
    ...
    async provideAvatar(repository, query) {
        ...
        const descriptor = (0, util_1.getRepositoryDefaultRemote)(repository);
        if (!descriptor) {
        ...
        try {
            ...
            const authorQuery = (0, util_1.groupBy)(query.commits, compareAvatarQuery);
            ...
            await Promise.all(authorQuery.map(async (commits) => {
                ...
                if (userIdFromEmail) {
                    ...
                    const avatarUrl = (0, links_1.getAvatarLink)(userIdFromEmail, query.size);
    ...
    async provideHoverCommands(repository) {
        const url = (0, util_1.getRepositoryDefaultRemoteUrl)(repository);
        if (!url) {
            return undefined;
        }
        return [{
    ...
    _onDidCloseRepository(repository) {
        for (const remote of repository.state.remotes) {
            ...
            const repository = (0, util_1.getRepositoryFromUrl)(remote.fetchUrl);
            if (!repository) {
                continue;
            }
