...
class GitTimelineItem extends vscode_1.TimelineItem {
    ...
    setItemDetails(uri, hash, avatar, author, email, date, message, shortStat, remoteSourceCommands = []) {
        ...
        if (hash) {
            this.tooltip.appendMarkdown(`---\n\n`);
            this.tooltip.appendMarkdown(`[\`$(git-commit) ${(0, util_1.getCommitShortHash)(uri, hash)} \`](command:git.viewCommit?${encodeURIComponent(JSON.stringify([uri, hash]))} "${vscode_1.l10n.t('Open Commit')}")`);
            this.tooltip.appendMarkdown('&nbsp;');
    ...
    async provideTimeline(uri, options, _token) {
        ...
        if (options.cursor === undefined) {
            ...
            if (index) {
                ...
                item.iconPath = new vscode_1.ThemeIcon('git-commit');
                item.description = '';
                item.setItemDetails(uri, undefined, undefined, you, undefined, dateFormatter.format(date), repository_1.Resource.getStatusText(index.type));
                const cmd = this.commands.resolveTimelineOpenDiffCommand(item, uri);
                if (cmd) {
                    item.command = {
                        title: openComparison,
            ...
            if (showUncommitted) {
                ...
                if (working) {
                    ...
                    item.description = '';
                    item.setItemDetails(uri, undefined, undefined, you, undefined, dateFormatter.format(date), repository_1.Resource.getStatusText(working.type));
                    const cmd = this.commands.resolveTimelineOpenDiffCommand(item, uri);
                    if (cmd) {
                        item.command = {
                            title: openComparison,
