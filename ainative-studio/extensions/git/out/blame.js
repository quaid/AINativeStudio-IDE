...
class GitBlameController {
    ...
    formatBlameInformationMessage(documentUri, template, blameInformation) {
        ...
        const templateTokens = {
            ...
            subject: (0, emoji_1.emojify)(subject ?? ''),
    ...
    async getBlameInformationHover(documentUri, blameInformation) {
        ...
        // Subject | Message
        markdownString.appendMarkdown(`${(0, emoji_1.emojify)(commitMessageWithLinks ?? commitInformation?.message ?? blameInformation.subject ?? '')}\n\n`);
        markdownString.appendMarkdown(`---\n\n`);
        // Short stats
...
class GitBlameEditorDecoration {
    ...
    _onDidChangeBlameInformation() {
        ...
        const config = vscode_1.workspace.getConfiguration('git');
        const template = config.get('blame.editorDecoration.template', '${subject}, ${authorName} (${authorDateAgo})');
...
class GitBlameStatusBarItem {
    ...
    async _onDidChangeBlameInformation() {
        ...
        if (typeof blameInformation[0].blameInformation === 'string') {
            ...
            const config = vscode_1.workspace.getConfiguration('git');
            const template = config.get('blame.statusBarItem.template', '${authorName} (${authorDateAgo})');
            this._statusBarItem.text = `$(git-commit) ${this._controller.formatBlameInformationMessage(vscode_1.window.activeTextEditor.document.uri, template, blameInformation[0].blameInformation)}`;
